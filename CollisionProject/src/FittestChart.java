...
public class FittestChart extends Application {
	...
	@Override public void start(Stage stage) {
		...
		ArrayList<Double> fitness = parse(this.getParameters().getRaw().get(0));

		stage.setTitle("Highest Fitness Chart");
		//defining the axes
		...
		lineChart.setTitle("Highest Fitness vs. Generation");
		//defining a series
		XYChart.Series<Number, Number> series = new XYChart.Series<Number, Number>();
		series.setName("Highest Fitness");
		//populating the series with data
		for(int i = 0; i < fitness.size(); i++)
			series.getData().add(new XYChart.Data<Number, Number>(i+1, fitness.get(i)-OFFSET));
